...
# 在外面跑好的数据
read_data = pd.read_csv('data_input/2moons.csv',header=None)
data_embedded = np.squeeze(read_data.iloc[:,:2].values)
labels = np.squeeze(read_data.iloc[:,4:5].values)
...
row = (data_length * np.random.rand(data_length//200)).astype('int')
...
def writeData(mark=-1,list_selected=[]):
    ...
    data.to_csv('data_output/2moons/'+usr_name+'.csv')
